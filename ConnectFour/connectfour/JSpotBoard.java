...
public class JSpotBoard extends JPanel implements SpotBoard {
	...
	private static final Color DEFAULT_BACKGROUND_LIGHT = Color.BLUE;
	private static final Color DEFAULT_BACKGROUND_DARK = Color.BLUE;
	private static final Color DEFAULT_SPOT_COLOR = Color.BLACK;
	private static final Color DEFAULT_HIGHLIGHT_COLOR = Color.YELLOW;

	private connectfour.Spot[][] _spots;
	...
	public JSpotBoard(int width, int height) {
		...
		_spots = new connectfour.Spot[width][height];
		...
		if (width == 3) {
			for (int y=0; y<height; y++) {
				for (int x=0; x<width; x++) {
					...
					_spots[x][y] = new connectfour.JSpot(bg, DEFAULT_SPOT_COLOR, DEFAULT_HIGHLIGHT_COLOR, this, x, y);
					((connectfour.JSpot)_spots[x][y]).setPreferredSize(preferred_size);
					add(((connectfour.JSpot) _spots[x][y]));
				}
			}
		} else if (width == 7) {
			for (int y=0; y<height; y++) {
	...
	@Override
	public connectfour.Spot getSpotAt(int x, int y) {
	...
	@Override
	public void addSpotListener(connectfour.SpotListener spot_listener) {
